...
struct button_s
{
	GPIO_TypeDef* GPIOx; //porta del pulsante
	uint16_t GPIO_Pin; //pin del pulsante
	button_state_t state; //stato attuale del pulsante, alto o basso
	button_state_t last_state;
	uint32_t delay; //ritardo del pulsante, 
	uint32_t last_tick;
};
...
int8_t button_read_pressed(const button_t* button, uint16_t* pin);
int8_t button_t_reset_pressed(const button_t* button);
int8_t button_t_pressed_handler(const button_t* button, uint16_t pin);
